...
export function V2Compare({ids}:{ids:string[]}){
  ...
  const rows=[
    ...
    ["Confirmed receipts",`${left?.settlement_legs.filter(item=>item.state==="CONFIRMED").length??0}/${left?.settlement_legs.length??0}`,`${right?.settlement_legs.filter(item=>item.state==="CONFIRMED").length??0}/${right?.settlement_legs.length??0}`],
  ];
