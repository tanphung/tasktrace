...
export function V2Report({deal}:{deal:V2Deal}){
  ...
  return <>
    ...
    <section className="v2-section" id="provenance"><header><div><span className="eyebrow">PROVENANCE</span><h2>Three artifacts, independently re-fetched</h2></div><span className="count-pill">{report.source_assessments.length}/3 verified</span></header><div className="v2-source-grid">
      {report.source_assessments.map(source=><article key={source.artifact_id}><div className="source-top"><span className="artifact-role">{source.artifact_id}</span><span className="verified"><CheckCircle2 size={14}/> Verified</span></div><strong>{source.path}</strong><p>{source.owner}/{source.repository}</p><dl><div><dt>Commit</dt><dd><code>{short(source.commit)}</code></dd></div><div><dt>Blob</dt><dd><code>{short(source.blob)}</code></dd></div><div><dt>SHA-256</dt><dd><code>{short(source.sha256)}</code></dd></div><div><dt>Bytes</dt><dd>{source.byte_length.toLocaleString()}</dd></div></dl><a href={github(source.owner,source.repository,source.commit,source.path)} target="_blank" rel="noreferrer">Open immutable artifact <ArrowUpRight size={13}/></a></article>)}
    </div></section>
