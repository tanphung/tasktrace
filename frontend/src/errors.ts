...
export function friendlyError(cause:unknown,fallback="The requested operation could not be completed"):string{
  ...
  if(/wrong network|switch (?:the )?wallet|chain mismatch|chain id/i.test(raw))return "The wallet is on the wrong network. Switch to the network shown in TaskTrace.";
