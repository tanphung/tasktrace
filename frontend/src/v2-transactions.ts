...
export async function submitV2(account:Address,dealId:string,method:string,args:CalldataEncodable[],value=0n){
  ...
  return navigator.locks.request(v2HistoryKey,{ifAvailable:true},async lock=>{
    ...
    const kit=createTransactionKit({chain,account,provider:wallet});
    ...
    const record:TxRecord={id:crypto.randomUUID(),jobId:dealId,method,account,chainId:chain.id,contract,value:String(value),phase:"SIGNING",createdAt:Date.now(),feeValue:String(quote.feeValue),feeSource:quote.source,feeVerification:quote.verification.status};
    save(record);
    try{
      const submitted=await kit.submit(quote,tx);
      record.hash=submitted.genlayerTxId as Hash;
